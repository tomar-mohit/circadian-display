...
class CurveRepository
    ...
    constructor(
        ...
        fun observeActiveProfile(): Flow<CurveProfile?> = profileDao.observeActive().map { it?.toDomain() }

        suspend fun getActiveProfile(): CurveProfile? = profileDao.getActive()?.toDomain()

        suspend fun getProfileById(id: Long): CurveProfile? = profileDao.getById(id)?.toDomain()

        suspend fun insertProfile(profile: CurveProfile): Long = profileDao.insert(profile.toEntity())

        suspend fun updateProfile(profile: CurveProfile) = profileDao.update(profile.toEntity())

        suspend fun deleteProfile(profile: CurveProfile) = profileDao.delete(profile.toEntity())

        suspend fun setActiveProfile(profileId: Long) = profileDao.setActive(profileId)

        suspend fun getProfileCount(): Int = profileDao.count()

        suspend fun getFirstProfile(): CurveProfile? = profileDao.getFirst()?.toDomain()
        ...
        suspend fun insertPoints(points: List<CurvePoint>) = pointDao.insertAll(points.map { it.toEntity() })

        suspend fun insertPoint(point: CurvePoint): Long = pointDao.insert(point.toEntity())

        suspend fun updatePoint(point: CurvePoint) = pointDao.update(point.toEntity())

        suspend fun deletePoint(point: CurvePoint) = pointDao.delete(point.toEntity())
